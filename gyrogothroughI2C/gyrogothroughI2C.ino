...
// class default I2C address is 0x68s
...
// Función que invoca cuando detecta una interrupción en el MPU 1
void dmp_1_DataReady() {
    mpuInterrupt_1 = true;
    Serial.println("Read 1 success");
}
// Función que invoca cuando detecta una interrupción en el MPU 2
void dmp_2_DataReady() {
    mpuInterrupt_2 = true;
    
    Serial.println("Read 2 success");
}
...
void setup() {
    ...
    if (devStatus_1 == 0) {
        ...
        attachInterrupt(0, dmp_1_DataReady, RISING); // We use the first external interrupt ( number 0 ) that is in digital pin 2
                                                     // When the interruption takes place invokes the " dmp_1_DataReady " function
                                                     // RISING triggers the interruption when the pin goes from high ( HIGH) to low ( LOW)
